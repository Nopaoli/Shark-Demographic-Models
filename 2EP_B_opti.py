...
def two_EP_b(params, ns):
    ...
    """
    nu1,nu2,s,T1,T2,m12a,m21a,m12b,m21b = params
    nu2_0 = nu1*s
...
# calculate the spectrum 
    ...
    fs.integrate([nu1, nu1*s], T1, m = numpy.array([[0, m12a], [m21a, 0]]))
    fs.integrate(nu_func, T2, dt_fac=0.01, m=numpy.array([[0, m12b], [m21b, 0]]))
    return fs


p_labels = "nu1,nu2,s,T1,T2,m12a,m21a,m12b,m21b"
upper = [20,20,0.999,10,10,100,100,100,100]
...
for i in range(2,6):
	...
	Optimize_Functions.Optimize_Routine(data, prefix, "two_EP_b", two_EP_b, 3, 9, data_folded=True, param_labels = p_labels, in_upper=upper, in_lower=lower, reps = reps, maxiters = maxiters, folds = folds)
