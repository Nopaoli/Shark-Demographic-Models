...
def usage():
    ...
    "# It uses the optimisation routine propose by Portick et al, 2017 for dadi and adapted to moments by Momigliano et al., 2021 \n\n"+
...
def takearg(argv):
   ...
   masked=False
   try:
      opts, args = getopt.getopt(argv,"hf:x:y:m:s:r:z",["fs_fil_name=","popname1=","popname2=","model=","folded=","replicat=","masked"])
   ...
   print ('Replicate of ', model,' number ',replicat)
   return(fs_fil_name, popname1, popname2, model, folded,replicat,masked )
...
# Load parameters
fs_fil_name, popname1, popname2,model, folded,replicat, masked= takearg(sys.argv[1:])

# Load the data
#Create python dictionary from SFS file

data = Spectrum.from_file(fs_fil_name) 
ns = data.sample_sizes
numpy.set_printoptions(precision=3)     
#if folded == 'True':
#    data = moments.Spectrum.fold(data) 
...
# matplotlib.savefig(fs_fil_namei+'.pdf')
...
if model =="two_ep":
    if folded == "True":
        ...
        Optimize_Functions.Optimize_Routine(data, prefix, "two_ep", Model_2pop_folded.two_ep, 4, 8, data_folded=True, param_labels = p_labels, in_upper=upper, in_lower=lower, reps = reps, maxiters = maxiters, folds = folds)
        if done: print ("\n" + model + "folded = "+folded+ " is done\n")
    if folded == "False":       
        ...
        Optimize_Functions.Optimize_Routine(data, prefix, "two_ep", Model_2pop_unfolded.two_ep, 4, 9, data_folded=False, param_labels = p_labels, in_upper=upper, in_lower=lower, reps = reps, maxiters = maxiters, folds = folds)
        if done: print ("\n" + model + "folded = "+folded+ " is done\n")


if model =="two_ep_b":
    if folded == "True":
        p_labels = "nu1, nu2, s,T1, T2,m12_0,m21_0,m12,m21"
        upper = [100,100,0.999,10,10,200,200,200,200]
        lower = [1e-3,1e-3,1e-3,1e-3,1e-3,1e-5,1e-5,1e-5,1e-5]
        Optimize_Functions.Optimize_Routine(data, prefix, "two_ep_b", Model_2pop_folded.two_ep_b, 4, 9, data_folded=True, param_labels = p_labels, in_upper=upper, in_lower=lower, reps = reps, maxiters = maxiters, folds = folds)
        if done: print ("\n" + model + "folded = "+folded+ " is done\n")
    if folded == "False":
        p_labels = "nu1, nu2, s,T1, T2,m12_0,m21_0,m12,m21,O"
        upper = [100,100,0.999,10,10,200,200,200,200,0.999]
        lower = [1e-3,1e-3,1e-3,1e-3,1e-3,1e-5,1e-5,1e-5,1e-5,1e-3]
        Optimize_Functions.Optimize_Routine(data, prefix, "two_ep_b", Model_2pop_unfolded.two_ep_b, 4, 10, data_folded=False, param_labels = p_labels, in_upper=upper, in_lower=lower, reps = reps, maxiters = maxiters, folds = folds)
        if done: print ("\n" + model + "folded = "+folded+ " is done\n")
...
if model =="two_ep_ae":
    if folded == "True":
        ...
        Optimize_Functions.Optimize_Routine(data, prefix, "two_ep_ae", Model_2pop_folded.two_ep_ae, 4, 10, data_folded=True, param_labels = p_labels, in_upper=upper, in_lower=lower, reps = reps, maxiters = maxiters, folds = folds)
        if done: print ("\n" + model + "folded = "+folded+ " is done\n")
    if folded == "False":
        p_labels = "nu_ae,nu1, nu2,Tae,T1,T2, m12, m21,O"
        upper = [100,100,100,10,10,10,200,200,200,200,0.999]
        lower = [1e-3,1e-3,1e-3,1e-3,1e-3,1e-3,1e-5,1e-5,1e-5,1e-5,1e-3]
        Optimize_Functions.Optimize_Routine(data, prefix, "two_ep_ae", Model_2pop_unfolded.two_ep_ae, 4, 11, data_folded=False, param_labels = p_labels, in_upper=upper, in_lower=lower, reps = reps, maxiters = maxiters, folds = folds)
        if done: print ("\n" + model + "folded = "+folded+ " is done\n")
...
if model =="two_ep_ae_b":
    if folded == "True":
        p_labels = "nu_ae,nu1, nu2,s,Tae,T1,T2, m12, m21"
        upper = [100,100,100,0.999,10,10,10,200,200,200,200]
        lower = [1e-3,1e-3,1e-3,1e-3,1e-3,1e-3,1e-3,1e-5,1e-5,1e-5,1e-5]
        Optimize_Functions.Optimize_Routine(data, prefix, "two_ep_ae_b", Model_2pop_folded.two_ep_ae_b, 4, 11, data_folded=True, param_labels = p_labels, in_upper=upper, in_lower=lower, reps = reps, maxiters = maxiters, folds = folds)
        if done: print ("\n" + model + "folded = "+folded+ " is done\n")
    if folded == "False":
        p_labels = "nu_ae,nu1, nu2,s,Tae,T1,T2, m12, m21,O"
        upper = [100,100,100,0.999,10,10,10,200,200,200,200,0.999]
        lower = [1e-3,1e-3,1e-3,1e-3,1e-3,1e-3,1e-3,1e-5,1e-5,1e-5,1e-5,1e-3]
        Optimize_Functions.Optimize_Routine(data, prefix, "two_ep_ae_b", Model_2pop_unfolded.two_ep_ae_b, 4, 12, data_folded=False, param_labels = p_labels, in_upper=upper, in_lower=lower, reps = reps, maxiters = maxiters, folds = folds)
        if done: print ("\n" + model + "folded = "+folded+ " is done\n")
...
if model == "two_ep_2M":
    if folded == "True":
        ...
        Optimize_Functions.Optimize_Routine(data, prefix, "two_ep_2M", Model_2pop_folded.two_ep_2M, 4, 11, data_folded=True, param_labels = p_labels, in_upper=upper, in_lower=lower, reps = reps, maxiters = maxiters, folds = folds)
        if done: print ("\n" + model + "folded = "+folded+ " is done\n")
    if folded == "False":
        ...
        Optimize_Functions.Optimize_Routine(data, prefix, "two_ep_2M", Model_2pop_unfolded.two_ep_2M, 4, 12, data_folded=False, param_labels = p_labels, in_upper=upper, in_lower=lower, reps = reps, maxiters = maxiters, folds = folds)
        if done: print ("\n" + model + "folded = "+folded+ " is done\n")


if model == "two_ep_b_2M":
    if folded == "True":
        p_labels = "nu1, nu2, s,T1, T2,m12_0,m21_0,m12,m21,i1,i2,P"
        upper = [100,100,0.999,10,10,200,200,200,200,0.999,0.999,0.499]
        lower = [1e-3,1e-3,1e-3,1e-3,1e-3,1e-5,1e-5,1e-5,1e-5,1e-3,1e-3,1e-3]
        Optimize_Functions.Optimize_Routine(data, prefix, "two_ep_b_2M", Model_2pop_folded.two_ep_b_2M, 4, 12, data_folded=True, param_labels = p_labels, in_upper=upper, in_lower=lower, reps = reps, maxiters = maxiters, folds = folds)
        if done: print ("\n" + model + "folded = "+folded+ " is done\n")
    if folded == "False":
        p_labels = "nu1, nu2, s,T1, T2,m12_0,m21_0,m12,m21,i1,i2,P,O"
        upper = [100,100,0.999,10,10,200,200,200,200,0.999,0.999,0.499,0.999]
        lower = [1e-3,1e-3,1e-3,1e-3,1e-3,1e-5,1e-5,1e-5,1e-5,1e-3,1e-3,1e-3,1e-3]
        Optimize_Functions.Optimize_Routine(data, prefix, "two_ep_b_2M", Model_2pop_unfolded.two_ep_b_2M, 4, 13, data_folded=False, param_labels = p_labels, in_upper=upper, in_lower=lower, reps = reps, maxiters = maxiters, folds = folds)
        if done: print ("\n" + model + "folded = "+folded+ " is done\n")
...
if model == "two_ep_ae_2M":
    if folded == "True":
        ...
        Optimize_Functions.Optimize_Routine(data, prefix, "two_ep_ae_2M", Model_2pop_folded.two_ep_ae_2M, 4, 13, data_folded=True, param_labels = p_labels, in_upper=upper, in_lower=lower, reps = reps, maxiters = maxiters, folds = folds)
        if done: print ("\n" + model + "folded = "+folded+ " is done\n")
    if folded == "False":
        ...
        Optimize_Functions.Optimize_Routine(data, prefix, "two_ep_ae_2M", Model_2pop_unfolded.two_ep_ae_2M, 4, 14, data_folded=False, param_labels = p_labels, in_upper=upper, in_lower=lower, reps = reps, maxiters = maxiters, folds = folds)
        if done: print ("\n" + model + "folded = "+folded+ " is done\n")


if model == "two_ep_ae_b_2M":
    if folded == "True":
        p_labels = "nu_ae,nu1, nu2,s,Tae,T1, T2,m12_0,m21_0,m12,m21,i1,i2,P"
        upper = [100,100,100,0.999,10,10,10,200,200,200,200,0.999,0.999,0.499]
        lower = [1e-3,1e-3,1e-3,1e-3,1e-3,1e-3,1e-3,1e-5,1e-5,1e-5,1e-5,1e-3,1e-3,1e-3]
        Optimize_Functions.Optimize_Routine(data, prefix, "two_ep_ae_b_2M", Model_2pop_folded.two_ep_ae_b_2M, 4, 14, data_folded=True, param_labels = p_labels, in_upper=upper, in_lower=lower, reps = reps, maxiters = maxiters, folds = folds)
        if done: print ("\n" + model + "folded = "+folded+ " is done\n")
    if folded == "False":
        p_labels = "nu_ae,nu1, nu2,s,Tae,T1, T2,m12_0,m21_0,m12,m21,i1,i2,P,O"
        upper = [100,100,100,0.999,10,10,10,200,200,200,200,0.999,0.999,0.499,0.999]
        lower = [1e-3,1e-3,1e-3,1e-3,1e-3,1e-3,1e-3,1e-5,1e-5,1e-5,1e-5,1e-3,1e-3,1e-3,1e-3]
        Optimize_Functions.Optimize_Routine(data, prefix, "two_ep_ae_b_2M", Model_2pop_unfolded.two_ep_ae_b_2M, 4, 15, data_folded=False, param_labels = p_labels, in_upper=upper, in_lower=lower, reps = reps, maxiters = maxiters, folds = folds)
        if done: print ("\n" + model + "folded = "+folded+ " is done\n")
